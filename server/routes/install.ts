  ...
  try {
    ...
    try {
      ...
      const [userResult] = await connection.execute<ResultSetHeader>(
        `INSERT INTO Users (Username, Email, PasswordHash, FirstName, LastName, UserType, IsAdmin, IsDeveloper, IsSupport, IsManager, IsActive, NavbarMenuLayout, NavbarLeftMode, NavbarLeftCollapsed) 
         VALUES (?, ?, ?, ?, ?, 'internal', 1, 1, 1, 1, 1, 'left', 'fixed', 1)`,
      ...
      for (const status of defaultTaskStatuses) {
        await connection.execute(
          `INSERT INTO TaskStatusValues 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [organizationId, status.name, status.color, status.order, status.isDefault, status.isClosed, status.isCancelled, status.hideFromPlanningAndStatistics]
        );
      }
