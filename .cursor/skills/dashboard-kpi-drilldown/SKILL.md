---
name: dashboard-kpi-drilldown
description: >-
  Keep Dashboard KPI card totals and drill-down details consistent (shared query
  source, navigable rows). Use when implementing or adjusting dashboard KPI
  values, details endpoints, or drill-down modals.
---
...
- Keep SQL provider-safe for **MySQL + MSSQL**.
- If KPI reads are cached, invalidate **optional Redis** on writes that change backing lists.
- Scope queries to **active org** where applicable.
- Match existing dashboard / **Synapse theme** card and modal styling (dark mode included).
...
11. Add or update unit tests under `__tests__/unit/` for shared query/parity logic.

## Related skills

Prefer matching skills under `.cursor/skills/` (e.g. `backend-route`, `frontend-feature`).
