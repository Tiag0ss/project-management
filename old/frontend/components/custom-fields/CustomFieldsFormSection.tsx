...
export default function CustomFieldsFormSection({
  ...
  // Fetch lookup rows for any custom-table-linked fields
  useEffect(() => {
    if (!token) return;
    const tableIds = [
      ...new Set(
        fields
          .filter((f) => f.CustomTableId)
          .map((f) => f.CustomTableId as number),
      ),
    ];
    ...
    const fetchRows = async () => {
      for (const tableId of tableIds) {
        if (tableRowsCache[tableId]) continue;
        try {
          const res = await fetch(`${getApiUrl()}/api/custom-tables/${tableId}/rows`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok && isMounted) {
            const data = await res.json();
            setTableRowsCache((prev) => ({ ...prev, [tableId]: data.rows || [] }));
          }
        } catch {
          // ignore
        }
  ...
  const groupedFields = useMemo(() => {
    ...
    const grouped = Array.from(groups.entries()).map(([key, groupFields]) => ({
      key,
  ...
  return (
    <div className={seamless ? 'space-y-6' : 'bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg'}>
        ...
        <div className="space-y-6">
          {groupedFields.map((group) => (
            <div key={group.key} className="space-y-4">
              {group.label && (
                <div className="pt-1 border-t border-gray-200 dark:border-gray-700 first:border-0">
                  <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-1">{group.label}</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {group.fields.map((field) => {
                  ...
                  if (field.CustomTableId) {
                    const rows = tableRowsCache[field.CustomTableId] || [];
                    return (
                      <div key={field.Id}>
                        {commonLabel}
                        <select
                          value={value === null || value === undefined ? '' : String(value)}
                          onChange={(event) => updateValue(field.FieldName, 'int', event.target.value ? Number(event.target.value) : null)}
                          required={isRequired}
                          disabled={disabled}
                          className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        >
